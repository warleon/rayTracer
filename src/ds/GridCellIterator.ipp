...
void GridCellIterator<T>::initialize() {
    ...
    if (intersection) {
        currentCell = grid->worldToGrid(ray.getOrigin() + intersection.value() * ray.getDirection());
    } else {
...
glm::ivec3 GridCellIterator<T>::getNextCellAlongRay() {
    ...
    // Compute the parametric t values at which the ray crosses the next cell boundary
